...
14. **Program Görüntüleme.** `lib/db/dutyAssignments.js`: `getAssignmentsForRange(supabase, schoolId, startDate, endDate)` — `teachers`/`duty_zones` join'li atama listesi. `app/(wizard)/schedule/`'a eklenen `fetchScheduleView` server action'ı + `ScheduleManager.jsx`'teki tarih×bölge tablosu: program oluşturunca otomatik gösteriliyor, ayrıca bağımsız bir "Görüntüle" butonuyla (yeniden üretmeden) mevcut bir aralık sorgulanabiliyor. Gün adları için `lib/engine/schedule.js`'teki mevcut `DAY_TR` sabiti yeniden kullanıldı (yeni bir kopya yazılmadı). Tarayıcıda uçtan uca doğrulandı: hem "Program Oluştur" sonrası otomatik tablo hem bağımsız "Görüntüle" doğru 5 satır gösterdi (tarih, gün, bölge, öğretmen adı hepsi doğru). Tam paket 83/83 yeşil.

### Bilinçli kararlar / teknik borç

- **Seçim ölçütü basit adillik (toplam nöbet sayısı), gerçek rotasyon değil.** `rotations` tablosundaki `zone_cursor`/`day_cursor`/`rotation_mode` (haftalık/aylık yer) henüz okunmuyor — kullanıcı onayıyla bu turun kapsamı dışında bırakıldı, ayrı bir ürün kararı gerektiriyor. **Bu, kalan tek büyük Faz 5 riski.**
...
- **Program Görüntüleme tablosu salt-okunur** — hücreye tıklayıp elle düzenleme (is_manual=true işaretleme) henüz yok. Bu, "kilitli hücre" konseptinin (CLAUDE.md ortak dili: `is_manual`) UI karşılığı — ayrı bir artış.
...
- Tam paket: 83/83 yeşil (Program Görüntüleme yalnızca tarayıcıda doğrulandı, yeni bir otomatik test eklenmedi — salt-okunur bir join sorgusu, mevcut `bulkSchedule.test.js` zaten aynı verinin doğru üretildiğini kanıtlıyor).
...
2. **Elle düzenleme (is_manual) UI'ı yok** → idareci motorun ürettiği bir atamayı değiştirip kilitleyemiyor → azaltma: Program Görüntüleme tablosuna hücre düzenleme eklenmeli.
