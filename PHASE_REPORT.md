...
- **`/schedule` ekranı sonucu sadece bir sayı gösteriyor** ("N nöbet ataması oluşturuldu"), üretilen programın kendisini (tablo/takvim görünümü) göstermiyor — bunun için idarecinin `duty_assignments`'ı ayrıca görüntüleyen bir ekrana ihtiyacı olacak (eski `(panel)` dashboard'daki "Program" sekmesinin yeni şemadaki karşılığı, henüz yok).
...
2. **Üretilen programı gösteren bir ekran yok** → idareci "5 atama oluşturuldu" mesajını görüyor ama kim-hangi-gün-hangi-bölge sorusuna cevap veren bir tablo/takvim yok → azaltma: bir sonraki artış bir "Program Görüntüleme" ekranı olabilir.
