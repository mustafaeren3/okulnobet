  ...
  return (
    <div className="dash-root" style={{ minHeight: 'auto' }}>
      <div className="stats-grid">
        <div className="stat-card"><span className="stat-icon">🏫</span><div><div className="stat-num">{schools.length}</div><div className="stat-lbl">Toplam Okul</div></div></div>
        <div className="stat-card"><span className="stat-icon">✅</span><div><div className="stat-num">{metrics.activeSchoolCount}</div><div className="stat-lbl">Aktif Okul</div></div></div>
        <div className="stat-card"><span className="stat-icon">🆓</span><div><div className="stat-num">{metrics.freeSchoolCount}</div><div className="stat-lbl">Free</div></div></div>
        <div className="stat-card"><span className="stat-icon">💼</span><div><div className="stat-num">{metrics.standardSchoolCount}</div><div className="stat-lbl">Standard</div></div></div>
        <div className="stat-card"><span className="stat-icon">🏢</span><div><div className="stat-num">{metrics.enterpriseSchoolCount}</div><div className="stat-lbl">Enterprise</div></div></div>
        <div className="stat-card"><span className="stat-icon">📈</span><div><div className="stat-num">%{Math.round(metrics.conversionRate * 100)}</div><div className="stat-lbl">Premium Dönüşüm Oranı</div></div></div>
        <div className="stat-card"><span className="stat-icon">💵</span><div><div className="stat-num">{formatTL(metrics.estimatedAnnualRevenue)}</div><div className="stat-lbl">ARR (projeksiyon)</div></div></div>
        <div className="stat-card"><span className="stat-icon">💰</span><div><div className="stat-num">{formatTL(Math.round(metrics.estimatedMonthlyRevenue))}</div><div className="stat-lbl">Normalize MRR (projeksiyon)</div></div></div>
        <div className="stat-card"><span className="stat-icon">🧾</span><div><div className="stat-num">{formatTL(totalCollected)}</div><div className="stat-lbl">Toplam Tahsilat (gerçek)</div></div></div>
        <div className="stat-card"><span className="stat-icon">🆕</span><div><div className="stat-num">{metrics.newThisWeekCount}</div><div className="stat-lbl">Son 7 Gün Kayıt</div></div></div>
        <div className="stat-card"><span className="stat-icon">✨</span><div><div className="stat-num">{totalGenerationCount}</div><div className="stat-lbl">Oluşturulan Program</div></div></div>
        <div className="stat-card"><span className="stat-icon">✅</span><div><div className="stat-num">{successRate !== null ? `%${successRate}` : '—'}</div><div className="stat-lbl">Program Üretme Başarı Oranı</div></div></div>
        <div className="stat-card"><span className="stat-icon">⏱️</span><div><div className="stat-num">{(avgGenerationDurationMs / 1000).toFixed(1)} sn</div><div className="stat-lbl">Ort. Oluşturma Süresi</div></div></div>
      </div>
      ...
      {criticalEvents.length > 0 && (
        <div className="card">
          <h3>🚨 Son Kritik Sistem Olayları</h3>
      ...
      <div className="card">
        <h3>🔥 En Aktif Okullar (üretim sayısına göre)</h3>
