-- ═══════════════════════════════════════════════════════════════
-- DÜZELTME: auth.users.email varchar(255) → text tip uyuşmazlığı
-- ═══════════════════════════════════════════════════════════════
-- Kök neden: auth.users.email sütunu character varying(255) tipinde,
-- text DEĞİL. RETURNS TABLE (... email text ...) bildiren bir plpgsql
-- fonksiyon, SELECT listesinde u.email'i açıkça ::text'e çevirmeden
-- döndürürse Postgres fonksiyon ÇAĞRILDIĞINDA (select * from fn() /
-- .rpc()) şu hatayı fırlatır:
--   ERROR 42804: structure of query does not match function result type
--   DETAIL: Returned type character varying(255) does not match
--   expected type text in column N.
-- Bu hata ham SELECT'i elle çalıştırınca ÇIKMAZ, sadece fonksiyon
-- olarak invoke edilince çıkar — bu yüzden önceki "okuma" tabanlı
-- doğrulamalar bunu yakalayamadı. jsonb dönen fonksiyonlar etkilenmez
-- (jsonb serileştirme varchar->text'i sessizce yutuyor).
--
-- Simüle edilmiş JWT testleriyle (set local request.jwt.claims) 3
-- fonksiyonda bu hata doğrulandı:
--   1. platform_get_active_impersonation() — owner.email (kolon 3)
--      Bu, impersonation'ın "gerçekte çalışmama" sebebiydi: bu RPC'nin
--      attığı hata lib/db/schoolContext.js'teki .catch(() => null) ile
--      sessizce yutuluyor, getSchoolForUser() hep null dönüyor, Dashboard
--      admini /super-admin'e geri yönlendiriyordu.
--   2. platform_list_schools_page() — owner.email (kolon 12) — Faz 2.2
--      Kullanıcı Yönetimi sayfası bu yüzden en baştan beri kırıktı.
--   3. platform_list_admins() — u.email VE gb.email (kolon 2 ve 4) —
--      Ayarlar sayfasındaki admin listesi, bu session'dan önce yazılmış
--      kod olmasına rağmen aynı sebepten kırıktı.
--
-- Tüm migration'lar tarandı (auth.users join eden + RETURNS TABLE
-- bildiren tüm fonksiyonlar); platform_list_schools(),
-- platform_list_purchase_intents(), platform_list_enterprise_leads()
-- (setof enterprise_leads — tablo sütunu zaten text, risk yok) ve
-- platform_get_school_detail() (jsonb dönüyor) etkilenmiyor.
...
begin
  ...
  return query
  select s.id, s.name, owner.email::text,
...
begin
  perform platform_require_admin();
  return query
  select pa.user_id, u.email::text, pa.role, gb.email::text, pa.revoked_at, pa.created_at
